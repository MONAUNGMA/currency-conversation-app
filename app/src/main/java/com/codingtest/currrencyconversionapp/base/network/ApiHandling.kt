...
abstract class BaseRemoteRepository {
    ...
    protected suspend inline fun <T> safeApiCall(
        ...
        return try {
            ...
            if (myObject != null) {
                Either.Right(myObject)
            } else {
//                dataException(ApiException(""))
            ...
            withContext(Dispatchers.Main) {
                ...
                when (e) {
                    is HttpException -> {
                        if (e.code() == 401) Either.Left(ApiException(ErrorType.SESSION_EXPIRED.name))
                        else {
                            val body = e.response()?.errorBody()
                            //  dataException(ApiException(getErrorMessage(body)))
                            Either.Left(ApiException(getErrorMessage(body)))
                        }
                    }
                    is SocketTimeoutException -> Either.Left(ApiException(ErrorType.TIMEOUT.name))
                    is IOException -> Either.Left(ApiException(ErrorType.NETWORK.name))
                    else -> Either.Left(ApiException(ErrorType.UNKNOWN.name))
                }
            }
        }
    }
